...
%Mik lehetnek a szimuláció kimenetelei? Type III nagy valószínűséggel kerül határciklusba vagy lesz instabil. \cite{martchenko2003investigating} --> meghatározták a stabilitás feltételét, mely csak a kapcsolati mátrixtól függ. \cite{tsadiras2008comparing} --> binary and trivalent FCMs can reach an equilibrium/fixed point or limit cycle, but only continuous state FCMs may behave chaotically.

%\begin{equation}
%\label{eq:fixedPoint}
%\exists t_0 \in \mathbb{N} : A_i^{t+1} = A_i^t, \forall t \geq t_0, i=1, \dots, n
%\end{equation}
